  ...
  <div class="card bg-base-100 shadow-xl">
    <div class="card-body">
      ...
      <div v-else-if="error && !profileData" class="alert alert-error">
        ...
        <button class="btn btn-sm" @click="loadProfile">Retry</button>
      </div>
      ...
      <div
        v-else-if="profileStore.isProfilesEnabled && profileFields.length === 0"
...
const props = withDefaults(defineProps<Props>(), {
  userId: '',
  // Absent means "fetch it from userId"; that is a distinct state from an
  // empty user object, so the default is undefined rather than {}.
  user: undefined,
})
...
function startEditing() {
  ...
  // `{ ...x }` is always truthy, so the `|| {}` that used to be here was
  // dead. Spreading `undefined` already produces `{}`.
