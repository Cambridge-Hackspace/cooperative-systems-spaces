    ...
    <div v-if="showForm" class="modal modal-open">
      <div class="modal-box max-w-2xl">
        ...
        <div class="form-control mb-3">
          ...
          <input
            v-model.number="form.unlock_duration_ms"
            type="number"
            min="500"
            max="60000"
            class="input input-bordered"
        ...
        <div v-if="!places.length" class="alert alert-warning mb-3 text-sm">
          You need at least one place to create a door. Add a place (or a special place like
          <span class="badge badge-warning badge-sm">Outside</span>) on the Places tab first.
        </div>
      ...
      <div class="modal-box max-w-3xl">
        ...
        <div role="tablist" class="tabs tabs-boxed mb-4">
          <a
            role="tab"
            class="tab"
            :class="{ 'tab-active': detailTab === 'settings' }"
          ...
          <a
            role="tab"
            class="tab"
            :class="{ 'tab-active': detailTab === 'rules' }"
            @click="detailTab = 'rules'"
            >Access rules</a
          >
          <a
            role="tab"
            class="tab"
            :class="{ 'tab-active': detailTab === 'events' }"
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class="form-control">
                <label class="label py-1"
                  ><span class="label-text">Unlock duration (ms)</span></label
                >
                <input
                  v-model.number="form.unlock_duration_ms"
                  type="number"
        ...
        <div v-else-if="detailTab === 'rules'">
            ...
            <tbody>
              <tr v-for="r in detail.rules" :key="r.id">
                ...
                <td>
                  <span
                    class="badge"
                    :class="r.effect === 'deny' ? 'badge-error' : 'badge-success'"
                    >{{ r.effect }}</span
                  >
                </td>
                <td class="text-xs">{{ scheduleLabel(r.schedule_id) }}</td>
                <td class="text-right">
                  <button class="btn btn-ghost btn-xs text-error" @click="removeRule(r)">
                    Remove
                  </button>
                </td>
              </tr>
          ...
          <div
            class="border-t border-base-300 mt-4 pt-3 grid grid-cols-1 md:grid-cols-5 gap-2 items-end"
          >
            <div class="form-control">
        ...
        <div v-else-if="detailTab === 'events'">
          <div v-if="!events.length" class="text-sm text-base-content/60">
            ...
            <tbody>
              <tr v-for="e in events" :key="e.id">
                ...
                <td class="text-xs">
                  ...
                  <span v-if="e.card_id_attempted" class="font-mono text-base-content/60">{{
                    e.card_id_attempted
                  }}</span>
                </td>
                <td>
                  ...
                  <span v-if="e.reason" class="text-xs text-base-content/60 ml-1">{{
                    e.reason
                  }}</span>
                </td>
              </tr>
        ...
        <div v-else-if="detailTab === 'qr'" class="text-center">
          <p class="text-sm text-base-content/70 mb-2">
            Print this and post it at the door. Members scan, sign in, and tap
          ...
          <img
            ...
            :src="qrDataUrl"
            alt="Door QR"
            class="mx-auto border border-base-300 rounded p-2 bg-white"
          />
