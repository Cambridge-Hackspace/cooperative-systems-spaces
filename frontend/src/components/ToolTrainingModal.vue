...
<!--            </button>-->
            ...
            <!-- OR embed it inline -->
            <button @click="showRecordTrainingForm = !showRecordTrainingForm" class="btn btn-success">
        ...
        <div v-if="isTrainer && !canManageTraining" class="trainer-section">
          ...
          <div class="trainer-actions">
            ...
            <button @click="viewTrainingHistory" class="btn btn-secondary">
        ...
        <div v-if="showTrainingHistory" class="training-history-modal">
          <div class="modal-content">
            <div class="modal-header">
              ...
              <button @click="showTrainingHistory = false" class="close-btn">&times;</button>
            </div>
            ...
            <div class="modal-body">
              ...
              <div v-else-if="trainingHistoryError" class="error">
                ...
                <button @click="viewTrainingHistory" class="btn btn-primary">Try Again</button>
              </div>
                    ...
                    <table>
                      ...
                      <tbody>
                        <tr v-for="record in trainingHistory" :key="record.id">
                          ...
                          <td>{{ record.minutes_trained ? record.minutes_trained + ' min' : 'N/A' }}</td>
        ...
        <div v-if="showRecordTrainingForm" class="record-training-form bg-primary text-primary-content">
          ...
          <form v-else @submit.prevent="submitRecordForm" class="record-form">
            <div class="form-row">
        ...
        <div v-else-if="error" class="error-message">
          ...
          <div class="error-actions">
            <button @click="loadTrainingOverview" class="btn btn-primary">
              Try Again
            </button>
            <button v-if="canManageTraining" @click="showSetupModal = true" class="btn btn-secondary">
              Set Up Training
            </button>
          </div>
        </div>

        <div v-else-if="!tool.requires_training || (!trainingOverview || trainingOverview.steps.length === 0)">
          <!-- No Training Steps -->
          <div class="no-training">
            ...
            <div v-if="canManageTraining" class="admin-actions">
              <p class="admin-note">As an administrator, you can set up training requirements for this tool.</p>
              <button @click="showSetupModal = true" class="btn btn-primary">
...
<!--                      </div>-->
                    ...
                    <div v-if="stepWithProgress.progress" class="progress-details">
                      <div v-if="stepWithProgress.progress.started_at" class="progress-item">
                        <strong>Started:</strong> {{ formatDate(stepWithProgress.progress.started_at) }}


                      </div>
                      <div v-if="stepWithProgress.progress.completed_at" class="progress-item">
                        <strong>Completed:</strong> {{ formatDate(stepWithProgress.progress.completed_at) }}


                      </div>
                      <div v-if="stepWithProgress.progress.expires_at" class="progress-item">
                        <strong>Expires:</strong> {{ formatDate(stepWithProgress.progress.expires_at) }}


                      </div>
                      <div v-if="stepWithProgress.progress.instructor_id" class="progress-item">
                        <strong>Instructor ID:</strong> {{ stepWithProgress.progress.instructor_id }}


                      </div>
                      <div v-if="stepWithProgress.progress.assessment_score !== null" class="progress-item">
...
<!--                    </div>-->
      ...
      <EditTrainingStepModal
        ...
        :step="selectedStep"
        :tool="tool"
        :existingSteps="trainingOverview?.steps?.map(s => s.step) || []"
...
const getStepNumberClass = (stepWithProgress: TrainingStepWithProgress): string => {
  ...
  switch (status) {
    case 'completed': return 'number-completed'
    case 'in_progress': return 'number-in-progress'
    case 'failed': return 'number-failed'
    case 'expired': return 'number-expired'
    default: return 'number-not-started'
  }
...
const getStatusBadgeClass = (stepWithProgress: TrainingStepWithProgress): string => {
  if (!stepWithProgress.progress) {
    return stepWithProgress.can_start ? 'badge-available' : 'badge-locked'
  }
...
const formatStepStatus = (stepWithProgress: TrainingStepWithProgress): string => {
  if (!stepWithProgress.progress) {
    return stepWithProgress.can_start ? 'Available' : 'Prerequisites Required'
  }
...
const viewAllProgress = () => {
  // TODO: Implement a modal or navigate to a progress view
  alert('View all users progress - to be implemented')
}
...
const useToolNow = () => {
  // TODO: Implement tool usage initiation
  alert('Tool usage initiation - to be implemented')
}
...
watch(() => showRecordTrainingForm.value, (newValue) => {
