...
| 6 Full stack | Does it work against a real database, broker, charset? | **Running, green.** Fourteen stages: preflight, up, schema, restart, contract, mfa, **mail**, fuzz, concurrency, health, devices, browser, logs, down. `mail` is the newest, and it earned its place on its first run: 19 cases, 3 failures, which turned out to be one product defect and two defects in the test that found it. The product defect was invisible in the source -- the reset message's `format!` continuation lines carried their own indentation into the body, so members received paragraphs indented thirteen spaces mid-sentence, and nothing but reading a message out of an SMTP sink could show it. The two test defects are the more instructive half: a `length >= 32` token assertion accepted a quoted-printable truncation that was exactly 32 characters, so one corrupted token surfaced as three failures downstream rather than one at the point of corruption; and `token-works-exactly-once` compared the second consume against 400 without checking the first had succeeded, so it **passed while proving nothing about single use at all**. It now skips with a message when its premise does not hold. All three fixed; the stage is 24/24 with no skips.

It also owns the only TLS handshake the mailer performs anywhere in this suite. The sink offers STARTTLS with a certificate `stack.sh` generates per run, and css-server trusts it through `SSL_CERT_FILE` -- so certificate verification stays **on**, which is the whole point: reaching TLS by switching verification off would assert that the mailer connects to anything claiming to be the relay. Before this, `use_tls = true` -- the transport every real deployment on port 587 uses -- was covered by a unit test asserting which *enum variant* got selected. A recent run put 54 messages over STARTTLS and 2 over plaintext, the latter being the branch the driver reaches by rewriting the config and calling `POST /api/admin/reload-config`. That reload also settles a claim `mail.rs` makes in its own doc comment and nothing had tested: that it reads `[email]` per send, so an operator fixing SMTP settings does not have to restart the server. Postgres LATIN1 / lc_collate=C / lc_ctype=C, `TZ=America/Chicago`, mosquitto, and the real release binary. It found the migration this schema could not apply, the 401-for-a-role defect, and the 404 on every deep link. `devices` runs both edge binaries, which is the only way to exercise a `#[cfg]` branch; `logs` treats the server's own ERROR output as an oracle. `mfa` is the only stage that can answer whether a second factor actually gates the JWT, and it generates its TOTP codes from a second, independent implementation of RFC 6238 checked against the specification's own vectors — so an accepted code is two implementations agreeing, not the server agreeing with itself. |
...
| The device digest's argon2 *parameters* are unchecked | the `Params` value in `CardDigester::wire_digest` | Two oracles cover the digest's shape: `css_lib`'s `the_wire_digest_is_not_a_bare_hmac_of_the_code` fails if argon2 is swapped for the fast HMAC ten lines away, and `checks/tests/a_device_cannot_reverse_a_card_digest.rs` fails if an edge is handed anything but the pepper. Neither would notice `Params::default()` being replaced by a weakened cost -- the wire format is identical, the type is identical, and the only observable difference is how long a 2^32 search behind a stolen device's digests takes. Recorded rather than covered by a grep for `Params::default()`, which would pin a spelling and not a cost, and would pass the day someone writes the same defaults out by hand. The check that would mean something is a timing floor, and it belongs with #109's follow-up rather than bolted to a source oracle. |
