      ...
      <div v-else>
        ...
        <div class="flex justify-center mb-6">
          <div
            ...
            :class="{
              ...
              'badge-error':
                status.auth_status === 'denied' || status.auth_status === 'unauthenticated',
            }"
          >
            {{ statusText }}
          </div>
        ...
        <div v-if="status.auth_status === 'unauthenticated'" class="mt-6">
              ...
              <div class="form-control">
                ...
                <label class="label">
                  <span class="label-text-alt"
                    >Enter the 8-emoji invite code from your administrator</span
                  >
                </label>
              </div>
          ...
          <div v-if="activeTab === 'cli'">
            ...
            <p class="text-sm text-gray-500 mt-4">
              Obtain the invite code from your CS: Spaces Server administrator, then run the command
        ...
        <div v-if="status.auth_status === 'approved'" class="alert alert-success">
          <div>
            ...
            <p class="text-sm mt-1">
              This device is connected and communicating with the CS: Spaces Server.
            </p>
          </div>
        </div>
        ...
        <div v-if="status.auth_status === 'pending'" class="alert alert-warning">
          <div>
            ...
            <p class="text-sm mt-1">
              Your device registration is awaiting approval from an administrator.
            </p>
          </div>
        </div>
        ...
        <div v-if="status.auth_status === 'denied'" class="alert alert-error">
          <div>
            ...
            <p class="text-sm mt-1">
              Your device registration was denied. Please contact your administrator.
            </p>
          </div>
        </div>
      </div>
...
/**
 * What to show the operator when registration fails.
 *
 * `catch` hands over `unknown`, and two shapes actually arrive: an HTTP error
 * carrying the server's own message at `response.data.error`, and a transport
 * failure carrying only `message`. Narrowed rather than asserted, because the
 * previous `catch (error: any)` read `.response.data.error` off whatever it was
 * given -- so anything that was not an object produced the literal string
 * "Network error: undefined" on a kiosk screen.
 */
function registrationFailureMessage(error: unknown): string {
  if (typeof error === 'object' && error !== null) {
    const fromServer = (error as { response?: { data?: { error?: unknown } } }).response?.data
      ?.error
...
async function registerDevice() {
  ...
  try {
    ...
    if (result.success) {
      ...
      setTimeout(() => {
        // `void`: loadStatus has its own try/catch/finally and never rejects,
        // so this is deliberately not awaited rather than accidentally unhandled.
        void loadStatus()
      }, 2000)
    } else {
    ...
    registrationError.value = registrationFailureMessage(error)
  } finally {
